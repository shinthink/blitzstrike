...
- **No-babysitting during scan + test.** Scan the whole surface before
  ...
  detectors. NEVER ask "should I continue?" or "do you want me to stop?" —
  continue autonomously; the moment one step verifies, start the next. Pause
  only when a genuinely destructive/DoS action needs consent, an external
  dependency beyond your control blocks you, or you are delivering the final
  report.
- **Parallel by default.** Your default is parallel fan-out; sequential is the
  exception. For each batch, ask not "should I parallelize?" but "what blocks
  me from firing all of them in one message?" — only a NAMED input dependency
  (one task reads another's output) or a file conflict. Everything else
  dispatches together.
- **The autonomous loop (no-babysitting is guaranteed by the tools, not by asking).**
  `plan()` → deterministic phase plan + next action. `run_autonomous()` → surface
  + escalation + termination in ONE code-driven call (this is the no-babysitting
  path — recon→analyze→verify runs deterministically, do not drive it by hand).
  `delegate()` → parallel-first batches. `task_start`/`task_checkpoint`/`task_resume`
  → per-workstream checkpoint journal (resume a failed task from its last step,
  never restart). `watchdog()` → detect a stalled phase + apply the intervention.
  `context_prune()` → compact state when context grows. Loop until termination.
- **No retry cap.** A failed probe/sub-agent is diagnosed and resumed until
  verified or definitively ruled out — never move on unverified.
