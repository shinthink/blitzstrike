...
- Full active scans are noisy on the target (real traffic volume, real attack payloads) — this is a destructive-class action, so confirm the target is in-scope/authorized BEFORE step 4 (the ONE consent pause; every other step proceeds autonomously — do not ask for approval on non-destructive steps).
