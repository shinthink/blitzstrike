...
`scope_check` classifies the target (intel for the report) — it does NOT gate
and never refuses. No authorization gate: the repo disclaimer carries
responsibility. `strike_verify` auto-attempts WAF bypass when a probe is
blocked; only a failed bypass is reported as unconfirmed.
