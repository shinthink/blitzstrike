...
export async function dnsBrute(domain: string): Promise<{ found: Array<{ host: string; ips: string[] }>; source: string }> {
  ...
  for (const prefix of COMMON_SUBDOMAINS) {
    ...
    try {
      ...
      if (ips.length > 0) found.push({ host, ips });
    } catch { /* NXDOMAIN — not found */ }
  }
...
export async function liveRecon(target: string, includeActive = false): Promise<LiveReconResult> {
  ...
  if (dnsFindings.length > 0) nextSteps.push(`DNS-resolved ${dnsFindings.length} hosts (${dnsFindings.map((d) => d.host).slice(0, 8).join(", ")}${dnsFindings.length > 8 ? "…" : ""}) — each is a live subdomain to recon.`);
