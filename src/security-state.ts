 ...
 *
 *  Binary taint only answers "tainted or not". The real complex bugs are
 *  CONTEXT bugs: the input WAS sanitized, but for the wrong sink —
 *  `sanitize_text_field()` (HTML) fed to a SQL query, or `esc_html()` fed to
 *  `eval()`.
 *
...
const SANITIZERS: Record<string, Sanitizer> = {
  ...
  esc_textarea: { context: "html", level: "sanitized" },
  esc_js: { context: "js", level: "sanitized" },
  ...
  // sanitize_key is a FORMAT sanitizer (lowercase + strip), NOT a value
  // whitelist — sanitize_key('administrator') is still 'administrator'. Treating
  // it as "validated" would silently pass role/enum sinks.
  sanitize_key: { context: "key", level: "sanitized" },
  sanitize_email: { context: "email", level: "validated" },
  sanitize_file_name: { context: "filename", level: "validated" },
  sanitize_url: { context: "url", level: "validated" },
  sanitize_title: { context: "slug", level: "validated" },
  sanitize_title_with_dashes: { context: "slug", level: "validated" },
  sanitize_user: { context: "username", level: "validated" },
  filter_var: { context: "validated", level: "validated" },
  wp_validate_boolean: { context: "validated", level: "validated" },
};
