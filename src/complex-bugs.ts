...
function detectSsrF(code: string, file: string, out: ComplexFinding[]): void {
  const ls = lines(code);
  const re = /\b(file_get_contents|fopen|file|readfile|curl_setopt|curl_init|fetch|axios|requests\.(get|post|put)|urllib\.request|http_get|wp_remote_(get|post)|new\s+URL\s*\(|got\(|http\.(get|request))\s*\(/g;
  let m: RegExpExecArray | null;
  while ((m = re.exec(code)) !== null) {
    const ln = lineNo(code, m.index);
    const l = ls[ln - 1] ?? "";
...
function detectXxe(code: string, file: string, out: ComplexFinding[]): void {
  const ls = lines(code);
  const re = /\b(simplexml_load_string|simplexml_load_file|new\s+SimpleXMLElement|->loadXML\s*\(|->load\s*\(|XMLReader\s*->\s*XML\s*\(|libxml_parse)\s*\(?/g;
  let m: RegExpExecArray | null;
  while ((m = re.exec(code)) !== null) {
    const ln = lineNo(code, m.index);
    const l = ls[ln - 1] ?? "";
...
function detectSsti(code: string, file: string, out: ComplexFinding[]): void {
  const ls = lines(code);
  const re = /\b(render_template_string|Template\s*\(|res\.render\s*\(|ejs\.(render|compile)\s*\(|_\.template\s*\(|render_string|twig\.render|Jinja2\(|jinja2\.Template)\s*\(?/g;
  let m: RegExpExecArray | null;
  while ((m = re.exec(code)) !== null) {
    const ln = lineNo(code, m.index);
    const l = ls[ln - 1] ?? "";
